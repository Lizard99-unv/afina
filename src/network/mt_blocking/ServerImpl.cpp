...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            std::lock_guard<std::mutex> guard(m);
            if (client_sockets.size() == max_clients_number) {
                close(client_socket);
                continue;
            }
...
void ServerImpl::Worker(int client_socket)
{
    ...
    try {
        int readed_all = 0;
        int readed_currently = -1;
        char client_buffer[4096] = "";
        while ((readed_currently = read(client_socket, client_buffer + readed_all, sizeof(client_buffer) - readed_all)) > 0) {
            _logger->debug("Got {} bytes from socket", readed_currently);
            readed_all += readed_currently;
            // Single block of data readed from the socket could trigger inside actions a multiple times,
            // for example:
            // - read#0: [<command1 start>]
            // - read#1: [<command1 end> <argument> <command2> <argument for command 2> <command3> ... ]
            while (readed_all > 0) {
                _logger->debug("Process {} bytes", readed_currently);
                // There is no command yet
                if (!command_to_execute) {
                    std::size_t parsed = 0;
                    if (parser.Parse(client_buffer, readed_all, parsed)) {
                    ...
                    if (parsed == 0) {
                        ...
                        std::memmove(client_buffer, client_buffer + parsed, readed_all - parsed);
                        readed_all -= parsed;
                    }
                }
                ...
                if (command_to_execute && arg_remains > 0) {
                    _logger->debug("Fill argument: {} bytes of {}", readed_all, arg_remains);
                    // There is some parsed command, and now we are reading argument
                    std::size_t to_read = std::min(arg_remains, std::size_t(readed_all));
                    argument_for_command.append(client_buffer, to_read);

                    std::memmove(client_buffer, client_buffer + to_read, readed_all - to_read);
                    arg_remains -= to_read;
                    readed_all -= to_read;
                }
...

    {
        ...
        close(client_socket);
        if (client_sockets.empty() && !running) {
            cv.notify_all();
        }
    }
}
